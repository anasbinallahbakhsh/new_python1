...
class Player:
    ...
    def draw(self, surf):
        ...
        body = WHITE if not flash else RED

        flick = abs(math.sin(self.engine_flicker))
        pygame.draw.polygon(surf, ORANGE, [
            (self.x - 10, self.y + self.radius),
            (self.x + 10, self.y + self.radius),
            (self.x, self.y + self.radius + 14 + flick * 10),
        ])
        pygame.draw.polygon(surf, YELLOW, [
            (self.x - 5, self.y + self.radius),
            (self.x + 5, self.y + self.radius),
            (self.x, self.y + self.radius + 8 + flick * 6),
        ])

        pygame.draw.polygon(surf, darken(TEAL, 10), [
            (self.x - self.radius, self.y + 8),
            (self.x - self.radius - 16, self.y + 22),
            (self.x - 6, self.y + 16),
        ])
        pygame.draw.polygon(surf, darken(TEAL, 10), [
            (self.x + self.radius, self.y + 8),
        ...
        pygame.draw.polygon(surf, body, [
            (self.x, self.y - self.radius - 14),
            (self.x - self.radius, self.y + self.radius),
            (self.x + self.radius, self.y + self.radius),
        ])
        pygame.draw.circle(surf, CYAN, (int(self.x), int(self.y - 2)), 7)
...
# --------------------------------------------------------------------------
ENEMY_TYPES = {
    "grunt": {"hp": 1, "points": 10, "radius": 16, "color": TEAL},
    "elite": {"hp": 2, "points": 25, "radius": 18, "color": PURPLE},
}
...
class Enemy:
    ...
    def draw(self, surf):
        ...
        flap = math.sin(self.bob * 2) * 6

        pygame.draw.ellipse(surf, darken(self.color, 20),
                             (x - self.radius - 6, y - 4 + flap, self.radius, self.radius * 0.7))
        pygame.draw.ellipse(surf, darken(self.color, 20),
                             (x + self.radius - self.radius * 0.4, y - 4 - flap, self.radius, self.radius * 0.7))

        pygame.draw.circle(surf, self.color, (int(x), int(y)), self.radius)
        pygame.draw.circle(surf, WHITE, (int(x - self.radius * 0.35), int(y - self.radius * 0.2)),
                            max(2, int(self.radius * 0.35)))
        pygame.draw.circle(surf, BLACK, (int(x - self.radius * 0.3), int(y - self.radius * 0.2)),
                            max(1, int(self.radius * 0.16)))
        pygame.draw.circle(surf, WHITE, (int(x + self.radius * 0.35), int(y - self.radius * 0.2)),
                            max(2, int(self.radius * 0.35)))
        pygame.draw.circle(surf, BLACK, (int(x + self.radius * 0.4), int(y - self.radius * 0.2)),
                            max(1, int(self.radius * 0.16)))
...
def reset_game():
    ...
    global score, wave_number, game_state
